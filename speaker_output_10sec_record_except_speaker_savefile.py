...
def record_audio(device_index, duration):
    ...
    return frames, FORMAT# p.get_format_from_width(FORMAT)

def save_audio(frames, format, file_name='output.wav'):
    CHANNELS = 1
    RATE = 44100
...
def play_audio(device_index, file_name='output.wav'):
    CHUNK = 1024
...
if __name__ == "__main__":
    ...
    if output_device_index is None:
        ...
        duration = 10
        recorded_audio, audio_format = record_audio(input_device_index, duration)
        save_audio(recorded_audio, audio_format)
        play_audio(output_device_index)